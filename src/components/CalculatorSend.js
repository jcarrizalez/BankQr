...
import {stringify, formated, evalAmount} from '../functions';
...
const CalculatorSend = () => {

  var [amount, setAmount] = useState('0');
  var [data] = useState(redux.get('route').data);
  ...
  const onPayment = () => {
    if(Number(amount)<=100){
      ...
      redux.push('isremote', true);
      redux.push('route', {
        component:'SendPaymentByQr',
        data:encodeURI(redux.get('link')+(encrypt(stringify(data.bank+'|'+data.number+'|'+data.phone+'|'+data.identifier+'|'+data.document+'|'+data.headline+'|'+amount))))
      });
    }
  }

  useEffect(() => {
    return () => redux.push('admobRewarded', true);
  },[]);
  ...
  return (
    <View style={css.calculator}>
      ...
      <View style={css.area}>
        ...
        <Bottom onPress={onPayment}>IR, HACER PAGO</Bottom>
      </View>
    </View>
  );
}
...
export default {
  screen: CalculatorSend,
  path: 'CalculatorSend/:name',
  navigationOptions: ({ navigation }) => ({
    ...defaultNavigationOptions,
      title: 'Indica un Monto a Enviar',
      headerLeft: <Back/>,
  }),
}